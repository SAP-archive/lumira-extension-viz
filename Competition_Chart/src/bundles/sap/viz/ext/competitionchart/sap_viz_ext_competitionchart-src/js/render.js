define("sap_viz_ext_competitionchart-src/js/render", [], function() {
	...
	var render = function(data, container) {
		...
		var lineY= {
			'x1':(function(d) { return x( 100 ); }),
			'y1':(function(d) { return y( yMax ); }),
			'x2':(function(d) { return x( 100 ); }),
			'y2':(function(d) { return y( yMin ); })
			};
       ...
       //Append circles 
		...
		svg.append("foreignObject")
	        ...
	        .on("click", function(d, i){
	        	...
	        	if(showLabels){
					...
					.attr("dx", function(d) { return x(d[ms1]); })
			    	.attr("dy", function(d) {
			    		var returnValue;
			    		if(additionalValue){
	    ...
	    dots.on("mouseover", function(d,i) {
			...
			var bubbleSizeText;	
			if(additionalValue){
				bubbleSizeText = "<tspan x=\""+(coordinates[0] + 20)+"\" dy=\"1.8em\"><tspan font-weight=\"bold\">"+ms2+": </tspan>"+(Math.round(d[ms2] * 10) / 10)+"%  ("+data[i][ms2] +")</tspan>";
			}else{
				bubbleSizeText = "";
			}
			...
			var text = tooltip.select('.sap_viz_ext_competitionchart_label_name')
				...
				.attr("y", coordinates[1] + 20)
			    .attr("font-size", "1.3em")
			    .html("<tspan x=\""+(coordinates[0] + 20)+"\" ><tspan font-weight=\"bold\">Name: </tspan>"+d[ds0]+"</tspan>"
			        + "<tspan x=\""+(coordinates[0] + 20)+"\" dy=\"1.8em\"><tspan font-weight=\"bold\">"+ms1+": </tspan>"+(Math.round(d[ms1] * 10) / 10)+"%  ("+data[i][ms1] +")</tspan>"
			        + "<tspan x=\""+(coordinates[0] + 20)+"\" dy=\"1.8em\"><tspan font-weight=\"bold\">"+ms0+": </tspan>"+(Math.round(d[ms0] * 10) / 10)+"%  ("+data[i][ms0] +")</tspan>"
			        + bubbleSizeText);
		...
		dots.transition()
	    	...
	    	.attr("cy", function(d) { return y(d[ms0]); })
	    	.attr("r", function(d) {
	    		var returnValue;
	    		if(additionalValue){
