...
/*dependency>>*/
	...
	var render = function(data, container) {
		//clean vars to avoid issues with resizing
		coords = [];
		power_countries = [];
...
// Replace the code below with your own one to develop a new extension
        ...
        try{
			...
			require(["runtime"], function(runt){
				...
				data.forEach(function(d){ powerByHash.set(d.from + d.to, d); });
				...
				vis.selectAll(".country")
					...
					.attr("d", path)
					.attr("onclick", function(d) { return "updateMap(\"" + d.id + "\");"; })
				...
				vis.append("path")
					.datum(topojson.mesh(eur, eur.objects.countries, function(a, b) { return a !== b; }))
					.attr("d", path)
					.attr("class", "country-boundary");
							
				//create a subset of all the countries to just those that are relevant for further drawing
				var selected_countries = topojson.feature(eur, eur.objects.countries).features.filter(function(d) {return power_countries.indexOf(d.id) > -1;});
				...
				*/
				vis.selectAll(".country-label")
					.data(selected_countries)
					.enter().append("text")
					.attr("class", function(d) { return "country-label " + d.id; })
					.attr("id", function(d) { return "label_" + d.id; })
					.attr("transform", function(d) { 
						var country_coords = {"id": d.id, "centroid": path.centroid(d), "bounds": path.bounds(d)};
				...
				powerByHash.forEach(function(key, val, map){
					var country_loc_from = coords.filter(function(d) { return d.id === val.from;});
					var country_loc_to = coords.filter(function(d) { return d.id === val.to;});
					if(val.from !== val.to){
						var ld = [{"x": country_loc_from[0].centroid[0], 
								   "y": country_loc_from[0].centroid[1]},
								  {"x": country_loc_to[0].centroid[0],
								   "y": country_loc_to[0].centroid[1]}];
						var fromto = {"key": key, "from": val.from, "to": val.to, "line": ld, "power": +val.POWER};
						//console.log(fromto);
...
var legendX;
		...
		function closestPoint(p, ps){
			...
			return {"x": cl_x, "y": cl_y};           
		}

		function findLineRatios(path, fromborder, toborder){
			var fromratio = 0.0, toratio = 1.0;
	...
	var updateMap = function(level){
			...
			var filterval = level;
			arrow_filtered = arrow_meta.filter(function(d){ return d.from === filterval ||  d.to === filterval; });
			...
			for(var n = 0; n<arrow_filtered.length; n++){
				...
				var to_meta = coords.filter(function(d){ return d.id === arrow_filtered[n].to;  });
				var from_meta = coords.filter(function(d){ return d.id === arrow_filtered[n].from;  });
			...
			info.append("text")
				.attr("x", 6)
				.attr("y", 12)
				.attr("class", "info_label")
				.attr("fill", "white")
				.text("Exports");
			
			var exports = arrow_filtered.filter(function(d){ return d.from === filterval; });
			var imports = arrow_filtered.filter(function(d){ return d.to === filterval; });
			/*
