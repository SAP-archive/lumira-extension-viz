define("sap_viz_ext_geomapsleaflet-src/js/render", [], function() {
	...
	var render = function(data, container) {
		...
		if (dset1.length === 3) {
			if (dset1[0].toLowerCase().substring(0, 3) === 'lat') {
				dimLat = dset1[0];
			} else if (dset1[1].toLowerCase().substring(0, 3) === 'lat') {
				dimLat = dset1[1];
			} else if (dset1[2].toLowerCase().substring(0, 3) === 'lat') {
				dimLat = dset1[2];
			}

			if (dset1[0].toLowerCase().substring(0, 3) === 'lon') {
				dimLon = dset1[0];
			} else if (dset1[1].toLowerCase().substring(0, 3) === 'lon') {
				dimLon = dset1[1];
			} else if (dset1[2].toLowerCase().substring(0, 3) === 'lon') {
				dimLon = dset1[2];
			}

			if (dset1[0].toLowerCase().substring(0, 3) !== 'lon' & dset1[0].toLowerCase().substring(0, 3) !== 'lat') {
				shapeName = dset1[0];
			} else if (dset1[1].toLowerCase().substring(0, 3) !== 'lon' & dset1[1].toLowerCase().substring(0, 3) !== 'lat') {
				shapeName = dset1[1];
			} else if (dset1[2].toLowerCase().substring(0, 3) !== 'lon' & dset1[2].toLowerCase().substring(0, 3) !== 'lat') {
				shapeName = dset1[2];
			}
		...
		function createMap(zoomLevel) {
			...
			function shadeColor1(color, percent) {
				var num = parseInt(color.slice(1), 16),
					...
					G = (num >> 8 & 0x00FF) + amt,
					B = (num & 0x0000FF) + amt;
				return "#" + (0x1000000 + (R < 255 ? R < 1 ? 0 : R : 255) * 0x10000 + (G < 255 ? G < 1 ? 0 : G : 255) * 0x100 + (B < 255 ? B < 1 ? 0 :
					B : 255)).toString(16).slice(1);
			}
