(function() {
    ...
    function render(data, vis, width, height, colorPalette, properties, dispatch) {
        ...
        x.domain(fdata.map(function(d) { return d.Dimensions[0]; }));
        ...
        y.domain([0, d3.max(fdata, function(d) {
            var maxMeasure =0, maxConf = 0;
            if(d.Measures!="") maxMeasure = +d.Measures;
            if(d.ConfidenceIntervals[1]!="") maxConf = +d.ConfidenceIntervals[1];
        ...
        for(i=0; i<fdata.length-1; i++) {
            if(fdata[i].Dimensions[2]=="Actuals"){
                if(!pendown){
                    dpath += "M";
                    pendown=true;
                }else{
                ...
                dpath+=x(fdata[i].Dimensions[0]);
                dpath = dpath + "," + y(+fdata[i].Measures);          
            }
        }
        ...
        dpath = "";
        for(i=0; i<fdata.length; i++) {
            if(fdata[i].Dimensions[2]=="Forecast"){
                if(!pendown){
                    dpath += "M";
                    pendown=true;
                }else{
        ...
        var ttitle = fdata[0].Dimensions[1];
        var model = "";
        for(i=0; i<fdata.length; i++) {
            if(fdata[i].Dimensions[2]=="Forecast"){
                model=fdata[i].Dimensions[3];
                break;
            }
        }

        vis.append("g").attr("class", "mytitle").append("text")
    ...
    (function() {/*__FOLD__*/
        ...
        var flowRegisterFunc = function(){
            ...
            var element  = sap.viz.extapi.Flow.createElement({
                id : 'comsapforecastModule',
                name : 'Forecast Chart Module',
            });
            element.implement('sap.viz.elements.common.BaseGraphic', moduleFunc);
            /*Feeds Definition*/
            //ds1: YearMonthString, ForecastBy, Type, Model
            var ds1 = {
                "id": "comsapforecastModule.DS1",
                "name": "Dimensions",
                "type": "Dimension",
                "min": 1,
                "max": 2,
            ...
            _util.mapping.dses.push("Dimensions");
            //ms1: Measure
            var ms1 = {
                "id": "comsapforecastModule.MS1",
                "name": "Measures",
                "type": "Measure",
                "min": 1,
                "max": Infinity,
            ...
            flow.addElement({
                'element':element,
                'propertyCategory' : 'comsapforecast'
            });
            sap.viz.extapi.Flow.registerFlow(flow);
        };
        flowRegisterFunc.id = 'comsapforecast';
