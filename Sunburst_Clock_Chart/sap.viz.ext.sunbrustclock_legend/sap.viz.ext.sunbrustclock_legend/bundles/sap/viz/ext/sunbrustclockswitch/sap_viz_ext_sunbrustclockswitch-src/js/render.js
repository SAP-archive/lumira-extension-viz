define("sap_viz_ext_sunbrustclockswitch-src/js/render", [], function() {
	/*
	...
	var render = function(data, container) {
		...
		var legendWidth = this.width()/8;
		var margin = {	top: 5, right: 5,	bottom: 5,	left: 5 },
			width = this.width() - margin.left - margin.right - legendWidth/2,
...
//		console.log(status + ", " + doorId + ", " + date + ", " + localTime+ ", " + status_secs);
		...
		var svg = container
			...
			.append("g")
			.attr("class", "sunbrustclock_legend")
		...
		// fixed title for day clock
		var innerData = [{ "key": "Door Shut", "value": 50, "text": ["PP%", "Door Shut"], "legend": "% TIME door remained shut"}, 
		                 { "key": "Door Cycles", "value": 50, "text": ["13", "Door Cycles"], "legend": "NUMBER of door OPEN + CLOSE cycles" }];
	   ...
	   var clockLegend = {"legend": "6 hour divisions for visualizing time"};

		// scale for sunburst data
		...
		var statusLegend = svg.append("foreignObject")
			.attr("width", legendWidth + margin.right).attr("height", height)
			.attr("transform", "translate(" + (outterRadius - 10) + "," + (0- height/2) + ")")
			// .attr("stroke", "gray" ).attr("stroke-width", "2")
			.append('xhtml:div')
            .attr("class","allStatusLegend");
    	...
    	var sunbrustStatus = root.children[0].children.map(function(d){ return d[status]; });
  		var sunbrustStatus2 = d3.nest()
			.key(function(d) {	return d; /* Incase there is space in stage name*/ })
			.sortKeys(d3.ascending)
			.rollup(function(d) { return d.length; })
			.entries(sunbrustStatus);
		var sunbrustColor = d3.scale.ordinal()
            .domain(sunbrustStatus2.map(function(d){ return d.key; }))
            .range(["#e5e059", "#bdd358", "#999799", "#e5625e", "#006ba6", "#0496ff", "#ffbc42", "#d81159", "#8f2d56"]);
		
		DrawLegend(statusLegend, sunbrustStatus2, "statusLegend");
    	...
    	var path = sunbrustChart.datum(root)
          ...
          .on('mouseover', function(d) {
                var selectpath = d3.select(this);
                if(Number(selectpath.attr("id")) > 1){
                    selectpath.attr('opacity', 0.8).attr("font-size", "1.4em").attr("font-weight", "bold");
                    var centroid = selectpath.node().getBBox();
                    ...
                    tipcontent.select('.project').attr("font-weight", "bold").attr("font-size", "1.2em")
                        .attr("dy","1.4em").attr("x", 10).text(d[status]);
    ...
    drawDayClock(dayClock, outterRadius, clockData, clockColor, "value", "text", 4, SimpleContent,"sunbrustclock_legend-bg");
    drawDayClock(doorShutInfo, innerRadius, innerData, innerColor, "value", "text", 2, MultiLineContent, "sunbrustclock_legend");
	...
	function DrawLegend(container, data, classname){
		var newLegend = container.append("div").attr("class", classname);
		
		for(var i in data){
			newLegend.append("div").attr("class", "key")
			.text(function(d){
				return data[i].key;
			})
			.append("span").style("width", "10px").style("height", "10px")
			.style("background", function(){
				var key = data[i].key;
				return key === "Open Limit"?"#fff":(key === "Close Limit"? "Transparent": sunbrustColor(key));
			});
		}
	}
